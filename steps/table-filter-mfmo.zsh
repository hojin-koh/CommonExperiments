...
description="Filter a table through another table and a perl expression (MFMO)"
...
main() {
  if [[ $#out != $#filt ]]; then
    err "Filter and Output must have the same number of parameters" 15
  fi

  local i
  for (( i=1; i<=$#out; i++ )); do
    info "Processing filter: ${filt[$i]}"
    local param="$(in::getLoader)"
    param+=" | uc/table-filter.pl ${(q+)filt[$i]} <($(infilt::getLoader))"
