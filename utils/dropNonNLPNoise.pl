...
# If there is no tab in the line, then we assume that document is empty, and just output the original id
...
while (<>) {
  ...
  $content =~ s/([^-\/_+\$%@#*\p{L}\p{N}\p{Z}\p{S}]|[●])/ /gu; # Only keep things useful to us
