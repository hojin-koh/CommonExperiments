...
dependencies=( "uc/prep/import-xlsx-table.py" "us/filter-twmoe-concised.pl" )
...
main() {
  ...
  uc/prep/import-xlsx-table.py $dirTemp/*.xlsx 1 2 1 \
  | perl -CSAD -nle 'print $_ . "\ttwmoe-concised"' \
  > $dirTemp/table.full

  outraw::save < $dirTemp/table.full

  us/filter-twmoe-concised.pl < $dirTemp/table.full \
  | sort -u \
  | out::save
  return $?
}
